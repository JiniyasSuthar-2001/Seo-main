...
class EvidenceReasoningProvider(BaseLLMProvider):
    ...
    def analyze(self, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        critical_issues = [
            i for i in issues 
            if isinstance(i, dict) and (i.get("severity") or "").lower() == "critical"
        ]
        if critical_issues:
            findings.append({
                ...
                "confidence": 0.98,
                "evidence": [
                    {"type": "affected_url", "value": i.get("affected_url")} 
                    for i in critical_issues[:3] if isinstance(i, dict) and i.get("affected_url")
                ],
                ...
                "affected_urls": [
                    i.get("affected_url") for i in critical_issues[:5] 
                    if isinstance(i, dict) and i.get("affected_url")
                ]
            })
        ...
        missing_h1_pages = [
            p for p in pages 
            if isinstance(p, dict) and not p.get("h1")
        ]
        if missing_h1_pages:
            findings.append({
                ...
                "confidence": 0.92,
                "evidence": [
                    {"type": "page_url", "value": p.get("url")} 
                    for p in missing_h1_pages[:3] if isinstance(p, dict) and p.get("url")
                ],
                ...
                "affected_urls": [
                    p.get("url") for p in missing_h1_pages[:5] 
        ...
        for p in pages:
            if isinstance(p, dict):
                wc = p.get("word_count")
                word_count_val = int(wc) if isinstance(wc, (int, float)) and wc is not None else 0
                
                sc = p.get("status_code")
                status_val = int(sc) if isinstance(sc, (int, float)) and sc is not None else 200

                if word_count_val < 150 and status_val == 200:
                    thin_pages.append(p)
        ...
        if thin_pages:
            findings.append({
                ...
                "evidence": [
                    ...
                    for p in thin_pages[:3] if isinstance(p, dict) and p.get("url")
                ],
                ...
                "affected_urls": [
                    p.get("url") for p in thin_pages[:5] 
                    if isinstance(p, dict) and p.get("url")
                ]
            })
        ...
        if not findings:
            findings.append({
                ...
                "severity": "Notice",
                "confidence": 0.95,
                "evidence": [{"type": "total_crawled", "value": context.get("pages_count", 0)}],
    ...
    def chat(self, query: str, context: Dict[str, Any]) -> str:
        ...
        domain = context.get("domain", "website")

        if "critical" in q_lower or "issue" in q_lower or "problem" in q_lower or "error" in q_lower:
            criticals = [i for i in issues if isinstance(i, dict) and (i.get("severity") or "").lower() == "critical"]
            warnings = [i for i in issues if isinstance(i, dict) and (i.get("severity") or "").lower() == "warning"]
            if not issues:
                return f"Based on the latest crawl snapshot for **{domain}**, zero technical issues were detected across **{context.get('pages_count', 0)}** pages."
        ...
        if "page" in q_lower or "url" in q_lower or "crawl" in q_lower:
            resp = f"The latest crawl of **{domain}** successfully fetched **{context.get('pages_count', 0)} page(s)**.\n\nSample crawled URLs:\n"
            for p in pages[:5]:
                if isinstance(p, dict):
