...
"""
            ...
            # Screaming-Frog-Style Crawl Data Layer (16 Datasets + SEO_Crawl_Data.xlsx)
            try:
                from app.services.crawl_data.crawl_tab_export_service import CrawlTabExportService
                target_project_id = kwargs.get("project_id") or (master_report.get("project", {}).get("id") if master_report else None) or domain
                crawl_csvs = CrawlTabExportService.export_all_tabs_csv_dict(
                    project_id=target_project_id,
                    domain=domain,
                    crawl_id=metadata.get("crawl_id")
                )
                for c_fname, c_content in crawl_csvs.items():
                    zf.writestr(f"{prefix}/crawl-data/{c_fname}", c_content)

                crawl_xlsx_bytes = CrawlTabExportService.export_crawl_data_xlsx(
                    project_id=target_project_id,
                    domain=domain,
                    crawl_id=metadata.get("crawl_id"),
                ...
                if crawl_xlsx_bytes:
                    zf.writestr(f"{prefix}/crawl-data/SEO_Crawl_Data.xlsx", crawl_xlsx_bytes)
            except Exception as cd_err:
                print(f"[ZIP EXPORT CRAWL DATA LAYER WARNING] {cd_err}", flush=True)
