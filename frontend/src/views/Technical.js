    ...
    async mounted() {
        ...
        try {
            ...
            if (this.activeTab === 'history') {
                ...
                const crawlsCount = histData.crawls_count || (compItems.length > 0 ? 2 : 0);
                const snapshots = histData.snapshots || [];

                const formatCrawlDate = (ts) => {
                    if (!ts) return '';
                    const d = new Date(ts);
                    return isNaN(d.getTime()) ? String(ts).split('T')[0] : d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
                };

                let tableHeadersHtml = '';
                if (crawlsCount >= 3) {
                    const ts1 = formatCrawlDate(snapshots[0]?.timestamp);
                    const ts2 = formatCrawlDate(snapshots[1]?.timestamp);
                    const ts3 = formatCrawlDate(snapshots[2]?.timestamp);
                    tableHeadersHtml = `
                        <th style="padding: 12px 18px; width: 28%;">Finding / Check</th>
                        <th style="padding: 12px; text-align: center;">
                            <div>Previous-Previous</div>
                            <div style="font-size: 10px; color: var(--text-tertiary); font-weight: 500;">${ts1}</div>
                        </th>
                        <th style="padding: 12px; text-align: center;">
                            <div>Previous</div>
                            <div style="font-size: 10px; color: var(--text-tertiary); font-weight: 500;">${ts2}</div>
                        </th>
                        <th style="padding: 12px; text-align: center;">
                            <div>Latest</div>
                            <div style="font-size: 10px; color: var(--primary); font-weight: 600;">${ts3}</div>
                        </th>
                        <th style="padding: 12px; text-align: center;">Status</th>
                        <th style="padding: 12px 18px;">Summary</th>
                    `;
                } else if (crawlsCount === 2) {
                    const ts1 = formatCrawlDate(snapshots[0]?.timestamp);
                    const ts2 = formatCrawlDate(snapshots[1]?.timestamp);
                    tableHeadersHtml = `
                        <th style="padding: 12px 18px; width: 32%;">Finding / Check</th>
                        <th style="padding: 12px; text-align: center;">
                        ...
                        <th style="padding: 12px; text-align: center;">
                            ...
                            <div style="font-size: 10px; color: var(--primary); font-weight: 600;">${ts2}</div>
                        </th>
                        <th style="padding: 12px; text-align: center;">Status</th>
                        <th style="padding: 12px 18px;">Summary</th>
                    `;
                    ...
                    tableHeadersHtml = `
                        <th style="padding: 12px 18px; width: 40%;">Finding / Check</th>
                        <th style="padding: 12px; text-align: center;">Latest Crawl</th>
                        <th style="padding: 12px; text-align: center;">Status</th>
                        <th style="padding: 12px 18px;">Summary</th>
                    `;
                }
                ...
                let tableRows = compItems.map(item => {
                    ...
                    if (st === 'RESOLVED') badgeStyle = 'background: rgba(16, 185, 129, 0.12); color: #10b981; border: 1px solid rgba(16, 185, 129, 0.3);';
                    else if (st === 'NEW') badgeStyle = 'background: rgba(239, 68, 68, 0.12); color: #ef4444; border: 1px solid rgba(239, 68, 68, 0.3);';
                    else if (st === 'IMPROVED') badgeStyle = 'background: rgba(59, 130, 246, 0.12); color: #3b82f6; border: 1px solid rgba(59, 130, 246, 0.3);';
                    else if (st === 'WORSENED') badgeStyle = 'background: rgba(245, 158, 11, 0.12); color: #f59e0b; border: 1px solid rgba(245, 158, 11, 0.3);';
                    else if (st === 'PERSISTENT') badgeStyle = 'background: rgba(139, 92, 246, 0.12); color: #8b5cf6; border: 1px solid rgba(139, 92, 246, 0.3);';

                    let cellsHtml = '';
                    if (crawlsCount >= 3) {
                        cellsHtml = `
                            <td style="padding: 12px; text-align: center; font-family: monospace;">${item.prev_prev_count !== null && item.prev_prev_count !== undefined ? item.prev_prev_count : '—'}</td>
                            <td style="padding: 12px; text-align: center; font-family: monospace;">${item.previous_affected_count !== null && item.previous_affected_count !== undefined ? item.previous_affected_count : '—'}</td>
                            <td style="padding: 12px; text-align: center; font-family: monospace; font-weight: 700; color: ${item.current_affected_count > 0 ? '#ef4444' : '#10b981'};">${item.current_affected_count}</td>
                        `;
                    } else if (crawlsCount === 2) {
                        cellsHtml = `
                            <td style="padding: 12px; text-align: center; font-family: monospace;">${item.previous_affected_count}</td>
                            <td style="padding: 12px; text-align: center; font-family: monospace; font-weight: 700; color: ${item.current_affected_count > 0 ? '#ef4444' : '#10b981'};">${item.current_affected_count}</td>
                        `;
                    } else {
                        cellsHtml = `
                            <td style="padding: 12px; text-align: center; font-family: monospace; font-weight: 700;">${item.latest_count || item.current_affected_count || 0}</td>
                        `;
                    }
                ...
                container.innerHTML = `
                    <div class="card" style="padding: 24px; border-radius: 14px; margin-bottom: 20px;">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 12px; flex-wrap: wrap; gap: 12px;">
                            <div>
                                <h3 style="font-size: 16px; font-weight: 700; margin: 0 0 4px 0; color: var(--text-primary);">Historical Crawl Comparison</h3>
                                <div style="font-size: 13px; color: var(--text-secondary);">
                                    ${this.escapeHtml(histData.message || "Compare audit findings across recent scans to track problem resolution.")}
                                </div>
                            </div>
                            <span class="badge badge-secondary" style="font-size: 11px;">
                                ${crawlsCount >= 3 ? '3-Crawl Timeline' : (crawlsCount === 2 ? '2-Crawl Comparison' : 'Initial Scan')}
                            </span>
                        </div>
                        
                        ${hasHistory && crawlsCount >= 2 ? `
                            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(170px, 1fr)); gap: 14px; margin-top: 18px;">
                                ...
                                <div class="kpi-card" style="padding: 16px; background: var(--bg-card); border-radius: 10px; border: 1px solid var(--border);">
                                    <div style="font-size: 11px; font-weight: 700; color: var(--text-secondary); text-transform: uppercase;">Persistent / Open</div>
                    ...
                    ${hasHistory ? `
                        <div class="card" style="padding: 0; overflow: hidden; border-radius: 14px;">
                            <div style="padding: 16px 20px; border-bottom: 1px solid var(--border); background: var(--bg-subtle);">
                                <h4 style="margin: 0; font-size: 15px; font-weight: 700; color: var(--text-primary);">Audit Findings Delta Timeline (${compItems.length})</h4>
                            </div>
                            <div style="overflow-x: auto;">
                                <table style="width: 100%; border-collapse: collapse; text-align: left; font-size: 13px;">
                                    ...
                                    <tbody>
                                        ${tableRows.length > 0 ? tableRows : `<tr><td colspan="6" style="padding: 32px; text-align: center; color: var(--text-secondary);">No findings recorded for this project.</td></tr>`}
                                    </tbody>
                                </table>
                            </div>
                        </div>
